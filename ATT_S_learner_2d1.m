function [result Y_estimated] = ATT_S_learner(covariates, T, Y, ind_of_treated, ind_of_untreated)
    ...
    x_with_2d1_t = [ones(size(x,1),1) x x.*T T];
    
    % fit a linear regression model
    b = regress(Y,x_with_2d1_t);
    
    % predict the outcome
    f_xi_t0 = [ones(size(x,1),1) x zeros(size(x)) zeros(size(x,1),1)]*b;
    f_xi_t1 = [ones(size(x,1),1) x x ones(size(x,1),1)]*b;
